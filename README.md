...
Draw a rectangle, record only that area to video, with everything outside the
selection dimmed like a spotlight. The dimmed overlay never appears in the
recording. Built and tested on Hyprland, designed for any wlroots-based
compositor (Sway, river, Wayfire).

## Demo

A short clip recorded with lanner itself will live here once input passthrough
lands (M4).
...
- Fullscreen spotlight overlay that dims everything outside your selection.
- Live rubber-band selection with a true transparent hole, so the recorded
  region stays clean and the dim is never captured.
...
- Planned: transcode to MP4, WebM, GIF, animated WebP, and AV1; audio source
  toggles; a countdown; on-overlay controls; desktop notifications.
...
Run it, drag a rectangle over the area you want, press Enter to start recording,
and press Esc to stop and save.
...
Bind it to a key in Hyprland (`~/.config/hypr/hyprland.conf`):

```
bind = SUPER, R, exec, /path/to/lanner
```
...
- [ ] M4: stop methods (overlay button, double-click outside, keybind toggle) and input passthrough
